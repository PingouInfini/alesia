...
export class PersonService {
  ...
  protected convertDateFromClient(person: IPerson): IPerson {
    const copy: IPerson = Object.assign({}, person, {
      dateOfBirth: person.dateOfBirth && person.dateOfBirth.isValid() ? person.dateOfBirth.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((person: IPerson) => {
        person.dateOfBirth = person.dateOfBirth ? moment(person.dateOfBirth) : undefined;
      });
    }
    return res;
  }
